  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return participantsAsync.when(
      ...
      error: (_, _) => _Message(
        ...
        action: FilledButton.icon(
          onPressed: () => ref.invalidate(matchParticipantsProvider(match.id)),
          icon: const Icon(Icons.refresh),
          label: const Text('Reintentar'),
        ),
      ),
      data: (rows) {
        final pending = rows
            .where((p) => p.participationStatus == ParticipationStatus.pending)
            .toList();
        final accepted = rows
            .where((p) => p.participationStatus == ParticipationStatus.accepted)
            .toList();
        final closed = rows
            .where(
              (p) =>
        ...
        return ListView(
          ...
          children: [
            ...
            _SectionTitle('Pendientes', count: pending.length),
            const SizedBox(height: 8),
            if (pending.isEmpty)
              _EmptyHint(text: 'No tienes solicitudes por responder.')
            else
